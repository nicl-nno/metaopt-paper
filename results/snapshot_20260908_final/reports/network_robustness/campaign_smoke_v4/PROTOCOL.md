...
Save within-task pair-order accuracy for each selected model on validation, excluding true-fitness ties and giving a predicted tie half credit. This uses no additional oracle calls, does not alter checkpoint selection, and is a diagnostic rather than an independent test outcome or a causal claim about the mechanism.
